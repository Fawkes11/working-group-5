...
const fechaActual = data.fechaActual;
  ...
  for (let i = 0; i < arrayHome.length; i++) {
    ...
    document.getElementById('eventosContainer').appendChild(div)
  ...
  function createLetters(id, objetoData) {
    ...
    <div class="card-body">
        ...
        <p class="card-text">${objetoData.description}</p>
    </div>
    <div class="informacion card-body ">
        <h2>$${objetoData.price}</h2>
        <a href="./pages/details.html" class="btn btn-primary" > Ver mas</a>
    </div>
    `
    return div
  }